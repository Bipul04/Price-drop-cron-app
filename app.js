...
function generateNewPrice(productPrice, drop_from, drop_to) {
...
const fetchShopifyData = async () => {
    try {
        ...
        const drop_from = data.fields.find(field => field.key === 'percentage_drop_from_data').value;
        const drop_to = data.fields.find(field => field.key === 'percentage_drop_to_data').value;
        ...
        setTimeout(() => {
            ...
            if (enable_price_drop === 'true') {
                ...
                cronJob = cron.schedule(`*/${timeBetweenPriceDrop} * * * * *`, () => {
                    console.log('Executing scheduled task every 10 seconds');
                    const newPrice = generateNewPrice(productPrice, drop_from, drop_to);
                    updateProductVariant(variant, parseInt(newPrice));
                }, {
                    scheduled: true,
